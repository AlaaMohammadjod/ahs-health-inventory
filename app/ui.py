import pandas as pd
from .constants import STOCK_RED_MAX, STOCK_ORANGE_MIN, STOCK_ORANGE_MAX, STOCK_GREEN_MIN
...
def format_stock_table(rows: list[dict]) -> pd.DataFrame:
    ...
    df["Level"] = df["on_hand"].apply(lambda x: stock_badge(int(x)))
    df["Availability"] = df["on_hand"].apply(lambda x: "Out of stock" if int(x) == 0 else "Available")

    df = df[["Level", "category", "item_name", "unit", "on_hand", "Availability", "reorder_threshold"]]
